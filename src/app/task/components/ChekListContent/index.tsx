...
import { CheckListProps, tasksProps } from '@/app/types/Types';

export default function Cheklist(dataProps:tasksProps){
    const [isOpenInput, setIsOpenInput] = useState(false);
    const [inputText, setInputText] = useState('');
    const [newArrayItems, setNewArrayItems] = useState<CheckListProps[]>([]);
    const param = useParams();
    const url = `http://localhost:3000/tarefas/${param.id}`;

    useEffect( () => {
        setNewArrayItems( dataProps.checklist?.map( item => item ) )
    }, [])
